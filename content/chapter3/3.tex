...
函数erase()和erase\_if()作为非成员函数定义在相应容器的头文件中。没有必要包含另一个头文件。
...
remove()函数的作用是:返回一个迭代器(remove\_it)，指向移位的元素之后的第一个元素。end()迭代器保持在remove()操作之前的状态。为了进一步说明，若要使用remove\_if()删除所有偶数元素，结果将如下所示:
