...
注意使用解引用操作符*从迭代器中访问元素，这和解引用指针的语法一样:
...
这些类别是分层级的，功能较强的迭代器继承功能较弱的迭代器的功能。换句话说，输入迭代器只能读取和递增一次。前向迭代器具有输入迭代器的功能，并且可以多次递增。双向迭代器具有这些功能，还可以自减。一次类推下。
...
\begin{tabular}[c]{@{}l@{}}迭代器可以由解引用操作符*读取。\\ 这包括指针、智能指针和输入迭代器。\end{tabular} \\ \hline
...
这个函数需要一个random\_access\_iterator。若用非随机访问容器的列表使用时，编译器会给报出错误:
...
列表迭代器类型不支持random\_access\_iterator概念。所以，编译器又报出了一个错误:
